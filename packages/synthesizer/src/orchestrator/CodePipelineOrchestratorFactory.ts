...
class CodePipelineOrchestratorStage implements Stage {
    ...
    addCodeCommitSourceAction(actionName: string, repositoryName: string, repositoryBranch: string): void {
        ...
        this.stage.addAction(new CodeCommitSourceAction({
            ...
            repository: Repository.fromRepositoryName(this.props.pipeline.codePipeline, `Repo${this.props.counter.next}`, repositoryName),
    ...
    addS3PublishAction(props: S3PublishProps): void {
        const bucket = props.action.BucketArn !== undefined ?
            s3.Bucket.fromBucketArn(this.props.pipeline.codePipeline, `Bucket${this.props.counter.next}`, props.action.BucketArn) :
            props.action.BucketName !== undefined ?
                s3.Bucket.fromBucketName(this.props.pipeline.codePipeline, `Bucket${this.props.counter.next}`, props.action.BucketName) :
        ...
        this.stage.addAction(new S3DeployAction({
            ...
            cacheControl: props.action.CacheControl !== undefined ?
                ...
                undefined,
            role: new Role(this.props.pipeline.codePipeline, `Role${this.props.counter.next}`, {
...
class CodePipelineOrchestrator implements Orchestrator {
    ...
    constructor(
        private readonly props: OrchestratorProps,
    ) {
        this.codePipeline = new CodePipeline(props.scope, `CodePipeline${props.counter.next}`, {
    ...
    addSources(): void {
        ...
        for (const source of tOrDefault(this.props.pipeline.Sources, [])) {
            ...
            sourceStage.addCodeCommitSourceAction(source.Name, source.RepositoryName, source.BranchName);
        }
    }
