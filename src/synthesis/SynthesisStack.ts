...
import {IPipelineConfigs, isCodeBuildAction} from "./IPipelineConfig";
import {IManagerResources} from "./SynthesisHandler";
...
export class SynthesisStack extends Stack {
    constructor(scope: Construct, id: string, managerResources: IManagerResources, pipelineConfigYaml: string, event: CodePipelineEvent, context: Context) {
        super(scope, id);

        const pipelineConfig = yaml.safeLoad(pipelineConfigYaml) as IPipelineConfigs
                ...
                for (const action of tOrDefault(stage.Actions, [])) {
                    ...
                    if (isCodeBuildAction(action)) {
                        const codePipelineProject = new Project(codePipeline, `${action.Name}Project`, {
                            ...
                            buildSpec: BuildSpec.fromObject(action.BuildSpec.Inline),
